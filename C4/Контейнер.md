...
Rel(spa, sing_in, "Вызывает api чтобы", "https")
Rel(spa, ide_load, "Вызывает api чтобы", "https")
Rel(spa, function_list, "Вызывает api чтобы", "https")
Rel(spa, trigger_list, "Вызывает api чтобы", "https")
...
Rel(external_user, Handler, "Отправляет", 'https')
Rel(Handler, db, "Читает из", 'https')

Rel_R(Handler, db_redis, "Отправляет задачу в")
Rel(db_redis, celery, "Отправляет в")
Rel_R(celery, docker, 'Использует')
